...
class DRIVEMODEL(nn.Module):
    """
    The model receive as input 5 consecutive images that have been captured
    ...
    this network consists of 3 modules:
        [*] A CNN (Resnet) that extract features from the images
        [*] A RNN (LSTM or GRU) that generates a representation of the sequence of features from the CNN
...
def save_model(model: DRIVEMODEL, save_dir: str, fp16, amp_opt_level: str = None) -> None:
    """
    Save model to a directory. This function stores two files, the hyperparameters and the weights.

    Input:
     - model: DRIVEMODEL model to save
...
def load_model(save_dir: str, device: torch.device, fp16: bool) -> DRIVEMODEL:
    """
...
def load_checkpoint(
...
) -> (DRIVEMODEL, str, torch.optim, torch.optim.lr_scheduler, float, int, bool, str):
    ...
    Output:
     - model: restored DRIVEMODEL model
     - optimizer_name: Name of the optimizer used for training: SGD or Adam
     - optimizer: Optimizer used for training
     - acc_dev: Accuracy of the model in the development set
     - epoch: Num of epoch used to train the model
